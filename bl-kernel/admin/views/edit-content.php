...
$hyttehubCanFeatureCabins = checkRole(array('admin'), false);
	...
	<div class="tab-content pr-3 pl-3 pb-3">
		...
		<?php
			...
			foreach ($customFields as $field=>$options) {
				if (($field === 'featured') && !$hyttehubCanFeatureCabins) {
					continue;
				}
...
<?php
	$customFields = $site->customFields();
	if (!$hyttehubCanFeatureCabins) {
		echo '<div class="alert alert-secondary mt-3 mb-2">Only admins can feature cabins on the homepage.</div>';
	}
	foreach ($customFields as $field=>$options) {
		if (($field === 'featured') && !$hyttehubCanFeatureCabins) {
			continue;
